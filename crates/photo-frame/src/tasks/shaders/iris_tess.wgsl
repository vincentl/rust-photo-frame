...
@fragment
fn fs_composite(in: FullscreenOut) -> @location(0) vec4<f32> {    let current = sample_plane(cur_tex, cur_samp, Comp.current_dest, screen_pos);
    let next = sample_plane(next_tex, next_samp, Comp.next_dest, screen_pos);
    // Binary mask: 0 → current, 1 → next
