...
const QUAD: [Vertex; 4] = [
    //   NDC pos         UV
    Vertex { pos: [-1.0, -1.0], uv: [0.0, 1.0] }, // bottom-left
    Vertex { pos: [ 1.0, -1.0], uv: [1.0, 1.0] }, // bottom-right
    Vertex { pos: [-1.0,  1.0], uv: [0.0, 0.0] }, // top-left
    Vertex { pos: [ 1.0,  1.0], uv: [1.0, 0.0] }, // top-right
];

pub fn run_slideshow(paths: Vec<PathBuf>) -> Result<()> {
...
impl App {
    ...
    fn current_path(&self) -> Option<&PathBuf> { self.list.get(self.cur_idx) }
    fn next_path(&self)    -> Option<&PathBuf> { self.list.get(self.next_idx) }
    ...
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        let first = self.current_path().cloned().and_then(|p| {
            image::open(&p).ok().map(|img| {
                let rgba = img.to_rgba8();
                let (w, h) = rgba.dimensions();
                Decoded { pixels: rgba.into_raw(), w, h, path: p }
            })
        });
        ...
        let gpu_init = async move {
            ...
            }).await.context("no compatible GPU adapter found")?;
            ...
            // texture helpers
            let make_tex = |device: &wgpu::Device, queue: &wgpu::Queue, w: u32, h: u32, pixels: &[u8]| -> Tex {
                ...
                Tex { view: tex.create_view(&wgpu::TextureViewDescriptor::default()), w, h }
            };
            ...
            let mut bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
                ...
                layout: &bind_layout,
                entries: &[
                    wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&tex_a.view) },
                    wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&sampler) },
                    wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(&tex_b.view) },
                    wgpu::BindGroupEntry { binding: 3, resource: wgpu::BindingResource::Sampler(&sampler) },
                    wgpu::BindGroupEntry { binding: 4, resource: params_a.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 5, resource: params_b.as_entire_binding() },
                    wgpu::BindGroupEntry { binding: 6, resource: fade_buf.as_entire_binding() },
                ],
            });
            ...
            let mut write32 = |buf: &wgpu::Buffer, data4: [f32; 4]| {
            ...
            WindowEvent::KeyboardInput { event, .. } => {
                if event.state == ElementState::Released {
                    ...
                    if let PhysicalKey::Code(code) = event.physical_key {
                        match code {
                            KeyCode::Escape | KeyCode::KeyQ => std::process::exit(0),
                            _ => {}
                        }
                    }
                }
            ...
            WindowEvent::Resized(PhysicalSize { width, height }) => {
                if let Some(gpu) = &mut self.gpu {
                    if width > 0 && height > 0 {
                        ...
                        gpu.queue.write_buffer(&gpu.params_a, 0, bytemuck::bytes_of(&block));
                        block = [0f32; 8];
                        block[0..4].copy_from_slice(&gpu.scale_b);
                        gpu.queue.write_buffer(&gpu.params_b, 0, bytemuck::bytes_of(&block));
                    }
                }
            }
    ...
    fn about_to_wait(&mut self, _el: &ActiveEventLoop) {
        ...
        if let Some(gpu) = &mut self.gpu {
            ...
            while let Ok(decoded) = self.rx_res.try_recv() {
                let new_b = upload_texture(&gpu.device, &gpu.queue, &decoded.pixels, decoded.w, decoded.h);
                gpu.tex_b = new_b;
                gpu.scale_b = compute_uv_scale(gpu.config.width, gpu.config.height, gpu.tex_b.w, gpu.tex_b.h);
            ...
            let now = Instant::now();
            if !self.fading && now.duration_since(self.last_switch) >= self.switch_interval {
                if gpu.tex_b.w > 1 || gpu.tex_b.h > 1 {
                    self.fading = true;
                    self.fade_start = now;
                    gpu.alpha = 0.0;
                    let mut fb = [0f32; 8];
                    fb[0] = gpu.alpha;
                    gpu.queue.write_buffer(&gpu.fade_buf, 0, bytemuck::bytes_of(&fb));
                }
            }
            ...
            if self.fading {
                ...
                if a >= 1.0 {
                    ...
                    gpu.queue.write_buffer(&gpu.params_b, 0, bytemuck::bytes_of(&block_b));
...
fn upload_texture(device: &wgpu::Device, queue: &wgpu::Queue, pixels: &[u8], w: u32, h: u32) -> Tex {
    let tex = device.create_texture(&wgpu::TextureDescriptor {
        label: Some("photo"),
        size: wgpu::Extent3d { width: w, height: h, depth_or_array_layers: 1 },
        mip_level_count: 1,
...
fn rebuild_bind_group(gpu: &mut Gpu) {
    gpu.bind_group = gpu.device.create_bind_group(&wgpu::BindGroupDescriptor {
        ...
        entries: &[
            wgpu::BindGroupEntry { binding: 0, resource: wgpu::BindingResource::TextureView(&gpu.tex_a.view) },
            wgpu::BindGroupEntry { binding: 1, resource: wgpu::BindingResource::Sampler(&gpu.sampler) },
            wgpu::BindGroupEntry { binding: 2, resource: wgpu::BindingResource::TextureView(&gpu.tex_b.view) },
            wgpu::BindGroupEntry { binding: 3, resource: wgpu::BindingResource::Sampler(&gpu.sampler) },
            wgpu::BindGroupEntry { binding: 4, resource: gpu.params_a.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 5, resource: gpu.params_b.as_entire_binding() },
            wgpu::BindGroupEntry { binding: 6, resource: gpu.fade_buf.as_entire_binding() },
        ],
    });
}
