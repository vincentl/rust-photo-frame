...
- The watcher generates three-word hotspot passwords from `/opt/photo-frame/share/wordlist_small.txt` and writes them to `/run/photo-frame/hotspot.env` for the templated hotspot unit. Passwords appear verbatim in the on-device UI and in no logs.【F:setup/files/wordlist_small.txt†L1-L1024】【F:crates/wifi-watcher/src/hotspot.rs†L14-L120】
